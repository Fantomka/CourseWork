...
class Matrix(list):
    ...
    def set_size_user_mode(self):
        ...
        while True:
            try:
                self.__dimension = int(input("Введите размерность матрицы, это целое число большее 1 и меньшее 7: "))
    ...
    def set_epsilon_user_mode(self):
        ...
        while True:
            try:
                self.epsilon = int(input('Задайте точность (количество чисел после запятой): '))
            except ValueError as error:
                print(error)
            else:
                break
